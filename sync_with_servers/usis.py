...
def before(filenames: list[str]) -> None:
    ...
    usis_sheet = get_sheet_by_name(
        state.info[InfoField.ENROLMENT_SHEET_ID], EnrolmentSprdsht.UsisBefore.TITLE
    )
    usis_data = usis_sheet.get_as_df(
        start=range_start[:-1], end=range_end, include_tailing_empty_rows=False
    )
    usis_data = usis_data.rename(
        columns={usis_data.columns[0]: EnrolmentSprdsht.UsisBefore.SECTION_COL}
    )
    ...
    usis_sheet.clear(start=range_start, end=range_end + end_row)
    update_sheet_values(
        {EnrolmentSprdsht.UsisBefore.RANGE + end_row: usis_data.values.tolist()},
        usis_sheet,
    )
    update_student_list()
...
def extract_students_from_xls_file(filename: str) -> tuple[int, pd.DataFrame]:
    ...
    print(
        FormatText.status(
            f"Student count: {FormatText.bold(section_students.shape[0])}"
        )
    )
    return (section_num, section_students)


def update_usis_dataframe_from_file(
    usis_data: pd.DataFrame, filename: str
) -> pd.DataFrame:
    section_num, section_students = extract_students_from_xls_file(filename)
    section_students.columns = usis_data.columns
    usis_data = usis_data[
        usis_data[EnrolmentSprdsht.UsisBefore.SECTION_COL] != section_num
    ]
    ...
    usis_data = usis_data.sort_values(
        by=[
            ...
            EnrolmentSprdsht.StudentList.STUDENT_ID_COL,
        ],
        ignore_index=True,
    )
    return usis_data
...
def update_student_list() -> None:
    ...
    state.df_student = get_sheet_data(
        state.info[InfoField.ENROLMENT_SHEET_ID], EnrolmentSprdsht.StudentList.TITLE
    )
    state.df_student = state.df_student.set_index(
        EnrolmentSprdsht.StudentList.STUDENT_ID_COL
    )
    ...
    state.df_marks_sec_lookup = state.df_student[
        [EnrolmentSprdsht.StudentList.DISCORD_ID_COL]
    ]
    # add a new column to track which section contains that student's marks
    state.df_marks_sec_lookup.insert(
        1, EnrolmentSprdsht.StudentList.VIRTUAL_MARKS_SEC_COL, 0
    )
    state.df_marks_sec_lookup.set_index(
        [state.df_marks_sec_lookup.index, EnrolmentSprdsht.StudentList.DISCORD_ID_COL],
        inplace=True,
    )
    # TODO: separate func to update df_marks_section. what if marks updates?
