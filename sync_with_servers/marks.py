...
def get_formatted_score_line(scores: pd.DataFrame, col_info: pd.DataFrame, 
                             marks_col: int) -> str:
...
def get_student_marks(sec: int, student_id: int, marks_col: int) -> str:
    ...
    marks = get_formatted_score_line(scores, col_info, marks_col)
    msg = MarksResponse.MAIN.format(marks)
    ...
    if children:
        ...
        for child_col in children:
            child_marks = get_formatted_score_line(scores, col_info, child_col)
            msg += MarksResponse.CHILD.format(child_marks)
    return msg
